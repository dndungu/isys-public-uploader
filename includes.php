...
class isys_visitor_posts {
	...
	public static function doAuthenticate(){
		global $wpdb;
		$sql = sprintf("SELECT `pid`, `username` FROM `wp_11_ngg_pictures` WHERE `username` = '%s' AND `password` = '%s'", $wpdb->prefix, $wpdb->escape(self::postString('author_username')), md5(self::postString('author_password')));
